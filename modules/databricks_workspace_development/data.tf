...
# IAM policy document
data "aws_iam_policy_document" "s3_acess_policy_root" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::${var.databricks_root_storage_bucket}/*",
      "arn:aws:s3:::${var.databricks_root_storage_bucket}",
    ]
    effect = "Allow"
  }
...
data "aws_iam_policy_document" "file_events_policy_root" {
  statement {
...
# IAM policy document
data "aws_iam_policy_document" "s3_acess_policy_external_development" {
  # S3 Bucket Permissions
  statement {
    actions = [
      "s3:GetObject",
    ...
    resources = [
      "arn:aws:s3:::${var.aws_development_bucket}/*",
      "arn:aws:s3:::${var.aws_development_bucket}",
    ]
    effect = "Allow"
  }
...
# This IAM policy grants Databricks permission to update your bucket's event notification configuration, 
# create an SNS topic, create an SQS queue, and subscribe the SQS queue to the SNS topic
data "aws_iam_policy_document" "file_events_policy_external_development" {
  statement {
    sid    = "ManagedFileEventsSetupStatement"
    effect = "Allow"
    actions = [
